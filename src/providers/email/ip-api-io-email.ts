...
export const ipApiIoEmail: Provider = {
  ...
  async lookup(query: string): Promise<ProviderResult> {
    ...
    try {
      ...
      if (wait > 0) return { provider: PROVIDER_NAME, success: false, data: {}, error: `Rate limited, retry in ${wait}s`, duration: Date.now() - start };

      const resp = await axios.get(`https://ip-api.io/api/v1/email/validate/${encodeURIComponent(query)}?api_key=${config.ipApiIoKey}`, {
