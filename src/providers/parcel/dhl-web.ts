  ...
  async lookup(query: string): Promise<ProviderResult> {
    ...
    try {
      ...
      if (sendung.sendungNichtGefunden?.keineDatenVerfuegbar) {
        return { provider: PROVIDER_NAME, success: false, data: {}, raw, error: 'No tracking data available for this number', duration: Date.now() - start };
      }
      ...
      const data: Record<string, unknown> = {
        ...
        parcel_type: details.kleinpaket ? 'Kleinpaket' :
                     details.expressSendung ? 'Express' :
                     details.quelle === 'PAKET' ? 'Paket' : details.quelle,
      };
      ...
      if (error?.response?.status === 429) {
        return { provider: PROVIDER_NAME, success: false, data: {}, error: 'Rate limited by DHL', duration: Date.now() - start };
      }
