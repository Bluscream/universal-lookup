...
export async function lookupParcel(query: string): Promise<ProviderResult[]> {
  ...
  if (providers.length === 0) {
    return [{ provider: 'parcel', success: false, data: {}, error: 'No parcel tracking providers available', duration: 0 }];
  }
  const results = await Promise.allSettled(
    providers.map(provider =>
      Promise.race([
        ...
        provider: provider.name, success: false, data: {},
        error: error instanceof Error ? error.message : String(error),
        duration: config.providerTimeout,
      }))
    )
  );
  return results.map(r => r.status === 'fulfilled' ? r.value : { provider: 'unknown', success: false, data: {}, error: 'Promise rejected', duration: 0 });
}
