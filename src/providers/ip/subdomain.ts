...
const COMMON_SUBDOMAINS = [
  ...
  'ns1', 'ns2', 'dns', 'mx', 'api', 'dev', 'staging', 'test',
  'admin', 'portal', 'vpn', 'remote', 'cdn', 'static', 'assets',
  'blog', 'shop', 'store', 'app', 'mobile', 'm', 'docs',
];
...
export const subdomainProvider: Provider = {
  ...
  async lookup(query: string): Promise<ProviderResult> {
    ...
    try {
      if (isIP(query)) {
        return { provider: PROVIDER_NAME, success: false, data: {}, error: 'Subdomain enumeration requires a domain, not an IP', duration: Date.now() - start };
      }
      ...
      if (crtResults.status === 'fulfilled') crtResults.value.forEach(s => found.add(s));
      if (dnsResults.status === 'fulfilled') dnsResults.value.forEach(s => found.add(s));
      const sorted = [...found].sort();
      return {
        provider: PROVIDER_NAME, success: sorted.length > 0,
        data: { subdomains: sorted },
        raw: { crt_sh: crtResults.status === 'fulfilled' ? crtResults.value : [], dns_brute: dnsResults.status === 'fulfilled' ? dnsResults.value : [] },
...
async function searchCrtSh(domain: string): Promise<string[]> {
  const resp = await axios.get(`https://crt.sh/?q=%25.${encodeURIComponent(domain)}&output=json`, { timeout: config.providerTimeout });
